...
//#define ASIM
//#define SKIP_VOICED_SAMPLES
//#define SKIP_UNVOICED_SAMPLES
//#define SKIP_CONTOUR
// CONTOUR added after interpolation
//#define SKIP_STRESS
//#define SKIP_INFLECTION
//#define FORCE_MONOTONE
...
unsigned char phonex[256];  //added by me
...
{
    int bit = lfsr & 1;
    return lfsr = (lfsr >> 1) ^ (bit ? 0x4001 : 0);    // LFSR
}
...
// handle buffer reordering from funny timing generated by samples
static void write_buffer(int i, int s)
{
    //buffer[i] = s;
    _buff[i & BMASK] = s;
    while ((i - _rpos) > 4)
        sam_raw(_buff[_rpos++ & BMASK]);
}

// Timing on output is really strange
...
void Output8BitAry(int index, unsigned char ary[5])
{
    ...
    if (bufferpos == 0) {
        _rpos = 0;
        memset(_buff,0x80,sizeof(_buff));
        //buffer[0] = buffer[1] = buffer[2] = 0x80;
    }
    ...
    for (k=0; k<5; k++)
        write_buffer(bufferpos/50 + k,ary[k]);
}
...
void RenderSample(unsigned char *mem66)
{
    ...
    Y = A ^ 255;
    if (debug)
        printf("Unvoiced sample 0x%02X, offset:%d->%d, len: %d\n",mem39,mem47*256+Y,(mem47+1)*256-1,256-Y);
...
// handle voiced samples here
   ...
   // number of samples?
    ...
    Y = *mem66;
    //if (debug)
    //    printf("Voiced samples %d, offset:%d, len: %d\n",mem39,mem47*256+Y,256-phase1);
...
//void sam_ple(const char* name, const uint8_t* data, int len);
...
do
{
    ...
    */
    ...
    Y = mem56;
   // printf("%d,%d,0x%2X,%d // %c%c index,pitch,flags,length\n",
  //         Y,(pitch+phase1) & 0xFF,sampledConsonantFlags[Y],phase2,signInputTable1[Y],signInputTable2[Y]);
    if (pitch == 0)
    {
        printf("how did pitch get zeros?\n");
    }
...
//   241     0     6     0    73     0    99    61
    ...
    while(1) //while No. 1
    {
        ...
        // pitch lerps between centers

        int a = mem49-phase1;
        int b = mem49+phase2;
        /*
        printf("at %d, blend between %d:%d, changing %d:%d, %c%c->%c%c a1:%d->%d a2:%d->%d\n",mark,a,b,a+1,b-1,
               signInputTable1[curr],signInputTable2[curr],
               signInputTable1[next],signInputTable2[next],
               amplitude1[mark],amplitude1[b],
               amplitude2[mark],amplitude2[b]
        );
         */

        // grr
        int run = a - mark;         // flat region
        int mix = b - a;            // mix count
        int ptch = pitches[mark];   // pitch? emphasis
        if (run < 0) {
            //printf("######## ERROR run < 0 this should not happen, run is %d\n",run);
            run = 0;
        }
        // map from 10ms to 16ms frames
        run = run/1.66666f + 0.5;
        mix = mix/1.66666f + 0.5;
        if (mix) {
            run++;
            mix--;      // first mix value is same as run
        }

        // don't mix between sampled TODO
        if ((sampledConsonantFlags[curr] & 0xF8) && (sampledConsonantFlags[next] & 0xF8))
            mix = 0;

        if (run > 31 || mix > 7)
            printf("######## ERROR run or mix overflow ######\n");
        if (run > 31)
            run = 31;
        if (mix > 7)
            mix = 7;
        /*
        printf("%d %c%c->%c%c run:%d, mix:%d\n", curr,
               signInputTable1[curr],signInputTable2[curr],
               signInputTable1[next],signInputTable2[next],
               run,mix);
        uint8_t frame[3] = {curr,(run << 3) | mix,ptch};
        if (run > 0)
            sam_ple("blend",frame,sizeof(frame));
         */
    ...
    // pause TODO
    // zero pitch is pause? when do we do that?
    int final_length = phonemeLengthOutput[mem44]/1.666666 + 0.5;
    if (final_length) {
        if (final_length > 31) {
            printf("############ ERROR final was > 31 (%d)\n",final_length);
            final_length = 31;
        }
        //uint8_t frame[3] = {curr,final_length << 3,0};
        //sam_ple("blend",frame,sizeof(frame));
    }
...
//
    ...
    mem44 = A;
    X = A;
    mem38 = A - (A>>2);     // 3/4*pitches[0];

if (debug)
{
    PrintOutput(sampledConsonantFlag, frequency1, frequency2, frequency3, amplitude1, amplitude2, amplitude3, pitches, mem48);
}
...
// reset at the beginning of each glottal pulse.
    ...
    while(1)
    {
        ...
        else
        {
            ...
            int k;
            for (k=0; k<5; k++) {
                signed char sp1 = (signed char)CLIPP(sinus[0xff & (p1>>8)]);
                signed char sp2 = (signed char)CLIPP(sinus[0xff & (p2>>8)]);
