...
void PrintPhonemes(unsigned char *phonemeindex, unsigned char *phonemeLength, unsigned char *stress)
{
    ...
    printf(" idx    phoneme  length  stress  inblend outblend\n");
    printf("-------------------------------------------------\n");
    ...
    unsigned char *p, int len)
{
    ...
    while(i < len)
    {
        uint8_t buf[8] = {flag[i], a1[i], f1[i], a2[i], f2[i], a3[i], f3[i], p[i]};
        //sam_ple("mixed",buf,8);
        printf("0x%02X, %5i, %5i, %5i, %5i, %5i, %5i, %5i, // %d\n", flag[i], a1[i], f1[i], a2[i], f2[i], a3[i], f3[i], p[i], i);
        i++;
    }
    printf("===========================================\n");
}
