...
unsigned char pitch_buffer[256];
unsigned char use_pitch_buffer = 0;
...
int SAMMain()
{
    ...
    SetPhonemeLength();
    AdjustLengths();
    Code41240();    // insert unvoiced consanant triples.
    do
    {
        A = phonemeindex[X];
        if (A > 80)
...
void Code41240()
{
    ...
    while(phonemeindex[pos] != 255)
    {
        ...
        if ((flags[index]&2) == 0)  // FLAG_STOPPED
        {
            pos++;
            continue;
        } else if ((flags[index]&1) == 0)  // FLAG_PLOSIVE
        {
