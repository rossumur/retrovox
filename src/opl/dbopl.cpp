...
void InitTables( void ) {
	...
	for ( Bitu i = 0; i < 32; i++ ) {
		...
		if ( i >= 16 )
			index += 9;
		Bitu blah = reinterpret_cast<unsigned long long>( &(chip->chan[ index ]) );
	...
	for ( Bitu i = 0; i < 64; i++ ) {
		...
		Bitu blah = reinterpret_cast<unsigned long long>( &(chan->op[opNum]) );
...
DBOPL::Handler _handler;

void opl3init_(int rate)
{
    _handler.Init(rate);
}

void opl3wr_(int r, int v)
{
    _handler.WriteReg(r,v);
}

int16_t* _buf;
static
void samples(Bitu n, Bit32s* s)
{
    for (int i = 0; i < n*2; i++)
        _buf[i] = *s++;
}

void opl3out_(int16_t *buf, int n)
{
    _buf = buf;
    _handler.Generate(0,samples,n);
}
