...
uint8_t _keyUp = 0;
uint8_t _lastkey = 0;
void ir_event(uint8_t ticks, uint8_t value); // t is HSYNCH ticks, v is value
...
void ir_apple(uint8_t t, uint8_t v)
{
  if (!v) {
    if (t > 32)
      _apple._state = 0;
...
void ir_flashback(uint8_t t, uint8_t v)
{
...
void ir_retcon(uint8_t t, uint8_t v)
{
  if (_retcon._state == 0)
  {
    if (v == 0)  {   // start bit
...
// key repeat logic is based on 60hz calling rate.....
// need some absolute timey thing
...
#define KEYDOWN     0x4A
#define KEYUP       0x5E
void ir_webtv(uint8_t t, uint8_t v)
{
  if (_state == 0)
  {
  ...
  else
  {
      ...
      if (bits == 16)
      {
        ...
        if (md == KEYDOWN) {
            _keyDown = _code;       // repeat rate is too high.. let higher layers deal with synthing repeat
        } else if (md == KEYUP) {
            _keyUp = _code;
        }
        _state = 0;                 // got one
