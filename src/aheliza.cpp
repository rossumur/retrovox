...
//template<typename T>
std::string pop_front(stringlist & container)
{
...
//template<>
char pop_front(std::string & container)
{
    auto v(container.front());
    container.erase(0, 1);
    return v;
...
std::string eliza(rulemap & rules, const std::string & input)
{
   ...
   // auto memory_rule = get_rule<rule_memory>(rules, SPECIAL_RULE_MEMORY);
    auto memory_rule = std::static_pointer_cast<rule_memory>(rules[SPECIAL_RULE_MEMORY]);
    if (keystack.empty()) {
    ...
    auto none_rule = std::static_pointer_cast<rule_vanilla>(rules[SPECIAL_RULE_NONE]);
...
{
    auto sleep = [](long ms) { std::this_thread::sleep_for(std::chrono::milliseconds(ms/2)); };
...
std::string getline()
{
    putchar('>');
    char buf[256];
    if (getline_s(buf,sizeof(buf)-1) < 0)
        return "EXIT";
    putchar('\n');
...
{
    try {
        std::stringstream script_file(script_text);
        elizascript::script s;
        elizascript::read<std::stringstream>(script_file, s);
