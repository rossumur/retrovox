...
typedef void (*opcode_P)(void);
const opcode_P op0_opcodes[0x10] PROGMEM =  {
    z_rtrue,
    z_rfalse,
    z_print,
    z_print_ret,
...
const opcode_P op1_opcodes[0x10] PROGMEM =  {
    z_jz,
    z_get_sibling,
    z_get_child,
    z_get_parent,
...
const opcode_P var_opcodes[0x40] PROGMEM =  {
    __illegal__,
    z_je,
    z_jl,
    z_jg,
...
const opcode_P ext_opcodes[0x1D] PROGMEM =  {
    z_save,
    z_restore,
    z_log_shift,
    z_art_shift,
...
#define CALL(_o) (*(_o))();
#else
#define CALL(_o) ((opcode_P)pgm_read_word(_o))();
#endif
#define CALL(_o) (*(_o))();
...
void call (zword routine, int argc, zword *args, int ct)
{
	...
	PUSH(pc >> 9);		/* for historical reasons */
    PUSH(pc & 0x1ff);	/* Frotz keeps its stack  */
    PUSH(fp - 1);		/* format compatible with */
    PUSH(argc | (ct << 8));	/* Mark Howell's Zip      */
...
void z_check_arg_count (void)
{
    ...
    if (fp == STACK_SIZE)
		...
		branch (zargs[0] <= (GETSTACK(fp) & 0xff));
